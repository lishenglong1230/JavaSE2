package Note0614_Array.ArrayTest01;
...
/*
    ...
    4、数据因为是引用类型，所以数组对象是堆内存当中。（数组是存储在堆当中的）
    ...
    12、数组中每一个元素都是有下标的，下表从0开始，以1递增。最后一个元素的下标是：length-1
    ...
    12、数组这种数据结构的优点和缺点是什么？
        优点：查询/查找/检索某个下标上的元素时效率极高。可以说是查询效率最高的一个数据结构。
            为什么检索效率高？
                ...
                第三：知道一个元素内存地址，知道每一个元素占用空间的大小，又知道下标，所以
                通过一个数学表达式就可以计算出某个下标上的元素的内存地址。直接通过内存地址
                定位元素，所以数组的检索效率是最高的。

                数组中存储100个元素，或者存储100万个元素，在元素查询/检索方面，效率是相同的，
                因为数组中元素查找的时候不会一个一个找，是通过数学表达式计算出来的。（算出一个内存地址，
                直接定位的。）
        缺点：
    ...
    15、怎么初始化一个一维数组呢？
        ...
        动态初始化语法格式：
            ...
            String[] array=new String[6];//初始化6个长度的String类型数组，每个元素默认值null。
...
public class  ArrayTest01 {
    public static void main(String[] args) {
        ...
        //所有的数组对象都有length属性
        System.out.println("数组中元素的个数"+a.length);//数组中元素的个数6

        //数组中每一个元素都有下标
        //通过下标对数组中的元素进行存和取。
        //取（读）
        System.out.println("第一个元素="+a[0]);//第一个元素=1
        System.out.println("最后一个元素="+a[5]);//最后一个元素=689
        System.out.println("最后一个元素="+a[a.length-1]);//最后一个元素=689
        ...
        System.out.println("最后一个元素="+a[5]);//最后一个元素=0
